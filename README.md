...
- **两种任务类型**（新增任务时在「任务类型」里选）：

  | 类型 | 适用场景 | 行为 |
  | --- | --- | --- |
  | **单志愿抢课** | 只想要**这一个**志愿，不知道容量何时释放 | 按填写的教学班ID轮询其所属类别，有容量就提交 |
  | **多志愿监控** | 有**多个**候选志愿，不知道**哪个**先释放容量 | 每行填一个教学班ID（可写 `教学班ID,类别代码`），每轮**只刷新它们所属的类别**并逐个检查容量；命中的**第一个**用最高优先级插队提交，其余按普通优先级；任意一个成功即结束，被业务拒绝则顺延下一个；**不因满课停止**（监控本身就是等放量） |

  - 「只刷新所属类别」= 同一类别的多个目标**只拉取一次**该类别，不做无谓重复请求；
  - 未写类别时会用**本地课程缓存**自动补全，缓存里也没有才退回任务默认类别；
  - 监控目标上限 `config.MONITOR_MAX_CLASSES`（20 个），自动去重；
  - 命中时使用的优先级 `config.PRIORITY_MONITOR_HIT = -10`，**高于**手动操作的
    `PRIORITY_HIGH = 0`，因此会插到队列最前面立刻提交；
