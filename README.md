...
**阶段：M5 进行中**（M1/M2/M4-逻辑/M3-数据层 已完成；M3 视觉方案与课表注入待侦察数据）

| 里程碑 | 内容 | 状态 |
| --- | --- | --- |
| M0 | 真机侦察（DOM / 站点库 / CSP / 课表页） | ⏸ 待用户在浏览器中执行（脚本内置自动侦察） |
| M1 | 脚手架、核心模块、悬浮窗 | ✅ 完成（待真机验证） |
| M2 | P0 抢课任务管理 | ✅ 完成（**写开关默认关闭**，待真机验证） |
| M3 | P1 课程列表增强 | ⏸ 待用户提供具体方案 |
| M3 | 数据层：字段容错归一化 + 被动旁听 + 课程缓存 | ✅ 完成 |
| M4 | P2 自定义课程 + 冲突计算（含站点课程） | ✅ 完成（待真机验证） |
| M4 | P2 课表页注入自定义课程 | ⏸ 待课表 DOM 侦察数据 |
| M5 | 页内自检 + 一键回传包 | ✅ 完成 |
| M5 | 发布方式、真实写请求验证 | ⏸ 未开始 |

## 一·五、怎么把问题反馈给我

脚本内置**诊断回传包**：面板 →「课程数据（诊断）」→「复制回传包」，一次得到

1. **页内自检**（在真实浏览器里验证整包是否完好，共 19 项不变量）
2. **环境侦察**（页面类型、站点库版本、DOM 计数、`sessionStorage` 结构；凭证已脱敏）
3. **课程数据**（采集到的课程逐条明细）
4. **原始响应样本**（最近 5 份真实响应 JSON）

剪贴板被拒时内容会自动落到日志面板供手动复制，不会静默失败。
